...
(function(){
  'use strict';
  function init(){
    var section=document.querySelector('#killswitch');
    ...
    var count=hud.querySelector('.ks-leak-count');
    ...
    if(!window.gsap||!window.ScrollTrigger){
      if('IntersectionObserver'in window){
        var io=new IntersectionObserver(function(entries){entries.forEach(function(e){if(e.isIntersecting){hud.style.opacity='1';hud.style.visibility='visible';io.disconnect();}});},{threshold:.2});
        io.observe(stage);
      }
      return;
    }
    ...
    tl.to(screen,{rotationX:0,rotationY:0,rotationZ:0,scale:1,y:0,duration:.22,ease:'none'},0)
      ...
      .to(count,{textContent:0,duration:.01},.78)
